...
get_data()
get_data_cmp()
figname_prefix = 'T_' + T_plot_str + '_' + folder_type1 + '_ratio4_'
...
#get_data()
#get_data_cmp()
#figname_prefix = 'highH_ratio4_'

speciesPlot_s = [
  [
     {'name': 'mH2O', 'drawVLineAt': 1.0/4},
     {'name': 'mHDO'},
     {'name': 'mHDO'},
     {'name': 'mD2O'},
  ],
...
for speciesPlot in speciesPlot_s:
  ...
  for j in xrange(n_files):
    ...
    for i in xrange(0,nSpeciesPlot-1,4):
      ...
      if speciesPlot[i]['idx'][j] == None:
        continue
      thislinestyle = linestyle[(i/4)%nlinestyle]
      thisratio4 = speciesPlot[i+2]['ratio'][j] / speciesPlot[i]['ratio'][j]
      ax.plot(dt['bin'][:,0], thisratio4,
        linestyle=thislinestyle,
        color=thiscolor, linewidth=linewidth)
      if 'drawVLineAt' in speciesPlot[i] and j == 0:
        ax.axhline(speciesPlot[i]['drawVLineAt'],
          linestyle=':', color='gray', linewidth=linewidth*2)
      if j == 0:
        label_str = '[' + getNameForPlot(speciesPlot[i+3]['name']) + \
            '/' + getNameForPlot(speciesPlot[i+2]['name']) + '] / [' + getNameForPlot(speciesPlot[i+1]['name']) + \
    ...
    if j == 0:
      lgd = ax.legend(loc='upper left', bbox_to_anchor=(0.01, 0.990),# borderpad=0.1,
    ...
    legend_2 = ax.legend(plot_2, ['n=' + scientific2latex('{0:7.1E}'.format(den)) + ' cm$^{{-3}}$' for den in densities],
               ...
               loc='upper left', fancybox=False, shadow=False)
    ax.add_artist(lgd)
