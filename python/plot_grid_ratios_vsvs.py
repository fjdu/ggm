...
fig_filename = 'grid_vsvs_' + '_'.join(tmp_list) + '.eps'
...
for speciesPlot in speciesPlot_s:
  ...
  for j in xrange(n_files):
    ...
    if 'multiply' in speciesPlot[0]:
      if speciesPlot[0]['multiply'] != 1e0 and iy==ny-1:
        xlabel = scientific2latex('{0:7.1E}'.format(speciesPlot[0]['multiply'])) + '$\\times$ ' + xlabel
    ...
    for i in xrange(2,nSpeciesPlot-1,2):
      ...
      ax.plot(speciesPlot[0]['ratio'][j], speciesPlot[i]['ratio'][j],
        ...
        #marker=markers[((i-1)/2)%nmarkers], markersize=markersize,
        marker='None',
      ...
      if j == 0:
        ...
        #ax.plot([],[],
        ax.plot(\
          [speciesPlot[0]['ratio'][j][-1]], [speciesPlot[i]['ratio'][j][-1]],
          linestyle='-',
          color=thiscolor, linewidth=linewidth,
          marker=markers[((i-1)/2)%nmarkers], markersize=markersize,
          markeredgewidth=markeredgewidth, markeredgecolor=markeredgecolor,
