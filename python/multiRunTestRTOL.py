...
execfile(os.path.join(pathAbsScript, 'multiRunPreProCommonTestRTOL.py'))
 ...
 for info1 in info2:
  for info in info1:
    ...
    if flag_use_old_moment == 0:
        ...
        copy_by_cmp(exe_src, exe_dest)

        exe_src_ini  = os.path.join(CommonFilesDir, src_exe_dir, file_exe_moment_ini)
        exe_dest_ini = os.path.join(running_dir, file_exe_moment_ini)
        copy_by_cmp(exe_src_ini, exe_dest_ini)
