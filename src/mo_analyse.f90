...
!./mo_analyse run_0.1/run_0.1_21.0_6.0E+05_1.0E-07_15.0_1.0E-01/config_moment_.dat species_ana.dat
     ...
     !      'C2H     ', 'C2H2    ', 'gNH3    ', 'gCH4    '/)
      ...
      integer sortByWhat ! 0: by accu rate; 1: by rate
      character(len=4) strSortByWhat
      double precision sumAccProdThis, sumAccDestThis
      ...
      CALL GET_COMMAND_ARGUMENT(3, strSortByWhat, i, ios)
      if ((ios .NE. 0) .OR. (len_trim(strSortByWhat) .EQ. 0)) then
        sortByWhat = 0
      else
        sortByWhat = 1
      end if
      ...
      do i=1, nSpeciesAna
        ...
        do j=2, nHistoryLen
          ...
          do k=1, nReactionContrib
            ...
            if (reactionAccum(k, j) .GT. 0D0) &
              sumAccProdThis = sumAccProdThis + reactionAccum(k, j)
            if (reactionAccum(k, j) .LT. 0D0) &
              sumAccDestThis = sumAccDestThis + reactionAccum(k, j)
          end do
          if (sortByWhat .EQ. 0) then
            CALL SORT_Asc_idx(nReactionContrib, &
              -abs(reactionAccum(:, j)), idxYdotOneReactionSorted)
          else
            CALL SORT_Asc_idx(nReactionContrib, &
          ...
          write (fU, '("[", I0, "] ", "Time: ", ES12.4, 2X, &
            ...
            "Total rate: ", ES10.2, 2X, &
            "vs ", ES10.2, 2X, "Prod,DestRate: ", 2ES10.2)') &
            ...
            sumAccProdThis, sumAccDestThis
            !yHistory(idxSpeciesAna, j)-yHistory(idxSpeciesAna, j-1)
          do k=1, min(nReactionContrib, 50)
            i1 = idxYdotOneReactionSorted(k)
            i2 = dtMM(2, i1, idxSpeciesAna)
            if (reactionAccum(i1, j) .GT. 0D0) then
              dblTmpVec(1) = reactionAccum(i1, j) / sumAccProdThis
            else if (reactionAccum(i1, j) .LT. 0D0) then
              dblTmpVec(1) = reactionAccum(i1, j) / sumAccDestThis
            else
              dblTmpVec(1) = 0D0
            end if
            write (fU, &
              '(2X, I3, ES11.2, 2X, ES11.2, "(", F4.2, "): " &
