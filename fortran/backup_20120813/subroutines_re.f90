      ...
      do i=1,nMantleSpecies
        if ((mantleSpe(i)%idxGas .GT. 0) .AND. &
            (mantleSpe(i)%idx_acc .GT. 0)) then
          tot_man_acc_rate = tot_man_acc_rate + &
            y(mantleSpe(i)%idxGas) * rates(mantleSpe(i)%idx_acc)
        end if
        if ((mantleSpe(i)%idxSurf .GT. 0) .AND. &
            (mantleSpe(i)%idx_eva .GT. 0)) then
          tot_man_eva_rate = tot_man_eva_rate + &
            y(mantleSpe(i)%idxSurf) * rates(mantleSpe(i)%idx_eva)
        end if
      end do
