      ...
      use CMTP
      implicit none
      integer i, i1, j, nImmoReac, nImmoProd
      logical flag
      n_surf_mant_inc = 0
      n_surf_mant_dec = 0
      do i=1, nGrReactions
        ...
        if ((.NOT. allowExposureByReaction) .AND. &
            (.NOT. IsGas(reactions(1+nReactants, i1)))) then
          cycle
        end if
        flag = .TRUE.
        do j=1, nRealReactants(i1)
          if (IsImmobile(reactions(j, i1))) then
            flag = .FALSE.
            exit
          end if
        end do
        do j=1, nRealProducts(i1)
          if (IsImmobile(reactions(j+nReactants, i1))) then
            flag = .FALSE.
            exit
          end if
        end do
        if (flag) cycle
        call get_num_surfImmo(i1, nImmoReac, nImmoProd) 
        if (nImmoReac .EQ. nImmoProd) then
          cycle
        else if (nImmoReac .LT. nImmoProd) then
          n_surf_mant_inc = n_surf_mant_inc + 1
          surf_mant_inc(n_surf_mant_inc)%idx = i1
          surf_mant_inc(n_surf_mant_inc)%gain_loss = &
            dble(nImmoProd - nImmoReac)
        else
          n_surf_mant_dec = n_surf_mant_dec + 1
          surf_mant_dec(n_surf_mant_dec)%idx = i1
          surf_mant_dec(n_surf_mant_dec)%gain_loss = &
            dble(nImmoReac - nImmoProd)
        end if
      end do
      ...
      subroutine get_num_surfImmo (iReac, nImmoReac, nImmoProd)
      use CMDT
      implicit none
      integer i, j, iReac, nImmoReac, nImmoProd
      nImmoReac = 0
      nImmoProd = 0
      do i=1, nRealReactants(iReac)
        if (IsImmobile(reactions(i, iReac))) &
          nImmoReac = nImmoReac + 1
      end do
      do i=1, nRealProducts(iReac)
        if (IsImmobile(reactions(i+nReactants, iReac))) &
          nImmoProd = nImmoProd + 1
      end do
      end subroutine get_num_surfImmo
      ...
      if (IsWordChar(fSpeciesSave(1:1))) then
        ...
        write(FMTstr, '("(A", I2, ", ", I2, "I4, ES14.5, 3ES11.2E4)")') &
