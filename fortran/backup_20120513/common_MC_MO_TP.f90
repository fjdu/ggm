...
integer, parameter :: dimvecVar=4, dimvecDeriv=8, &
  dimvecMon=32, dimvecMonlong=32, nameLen=1, nIterMoment=256
TYPE :: monomial
...
TYPE :: BranchInfo
  integer nSlave
  integer, dimension(:), allocatable :: idxSlave
  double precision, dimension(:), allocatable :: WeightSlave
END TYPE BranchInfo
...
logical, dimension(:), allocatable :: IsSurfRadical
...
type(BranchInfo), dimension(:), allocatable :: BranchInfoOnGrain
...
integer function getFstNoneRadical (n)
  implicit none
  integer n, getFstNoneRadical
  do getFstNoneRadical=1, nnzMM(n)
    if (IsGas(MM(getFstNoneRadical, n)) .OR. &
      .NOT. IsSurfRadical(MM(getFstNoneRadical, n))) then
      return
    end if
  end do
  getFstNoneRadical = 0
  return
end function getFstNoneRadical
...
integer function getFstDetSpe (n, y, NEQ)
  ...
  do getFstDetSpe=1, nnzMM(n)
    if (IsGas(MM(getFstDetSpe, n)) .OR. &
        (.NOT. IsStoSpecies(MM(getFstDetSpe, n))) .OR. &
        (.NOT. IsSurfRadical(MM(getFstDetSpe, n)))) then
...
logical function IsEqualSet (s1, s2, n)
  IMPLICIT NONE
  integer n, i
  ...
  t1 = s1; t2 = s2
  IsEqualSet = .TRUE.
  call SORT_Asc_in_place(n, t1)
  call SORT_Asc_in_place(n, t2)
  do i=1, n
...
end function IsEqualSet


integer function getReactionMM (iReac)
  use CMDT
  IMPLICIT NONE
  integer iReac, getReactionMM
  logical flag
  flag = .TRUE.
  do getReactionMM=1, nNumMM
    if (nnzMM(getReactionMM) .NE. nRealReactants(iReac)) cycle
    if (IsEqualSet(MM(1:nnzMM(getReactionMM), getReactionMM), &
        reactions(1:nRealReactants(iReac), iReac), &
        nnzMM(getReactionMM))) then
      flag = .FALSE.
      exit
    end if
  end do
  if (flag) getReactionMM = 0
  return
end function getReactionMM
