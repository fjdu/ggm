...
!gfortran common_MC_MO.f90 common_MC_MO_TP.f90 subroutines_share_triv.f90 mo.f90 subroutines_share_proc.f90 subroutines_mo.f90 opkd*.o -fbounds-check -o MO
!
      program moment_auto
      use CMDT
      use CMTP
      ...
      if (IsWordChar(fOutLog(1:1))) then
        if (.NOT. getFileUnit(fUOutLog)) then
          ...
          write (*,*) 'Cannot get an output file unit!'
          stop
        end if
...
! Import all the reactions
      ...
      if (statALLOC .ne. 0) then
        write(*,*) 'Error allocating strReactants, strProducts, ...'
        stop
      end if
      ...
      allocate ( &
         ...
         STAT=statALLOC)
      if (statALLOC .ne. 0) then
        write(*,*) 'Error allocating SpeciesElements, massSpecies, ...'
        stop
      end if
      ...
      if (statALLOC .ne. 0) then
        write(*,*) 'Error allocating mantleSpe.'
        stop
      end if
...
! Now I need to prepare all the possibly needed moments.
      ...
      write(*,*) 'dimvecDeriv, nNumMMEst=', dimvecDeriv, nNumMMEst
      ...
      allocate (&
        ...
        ndauMM(nNumMMEst), STAT=statALLOC)
      if (statALLOC .ne. 0) then
        write(*,*) 'Error allocating polyReac, MM, ...'
        stop
      end if
