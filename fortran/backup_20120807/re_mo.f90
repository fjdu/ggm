...
!
!gfortran common_MC_MO.f90 common_MC_MO_TP.f90 subroutines_share_triv.f90 re_mo.f90 subroutines_share_proc_all_tunnel.f90 subroutines_re_mo.f90 opkd*.o -fbounds-check -o RE_MO
...
845     if (ios .NE. 0) then
      ...
      ! end if
        ...
        if (((totalSurfSpe + totalMantleSpe)/SitesPerGrain) .GT. &
            numGrainLayerThreshold) then
          do j=1, nGrSpecies
            i1 = idxGrSpecies(j)
            ...
            if (((yHistory(i1, i) .GE. sto_threshold*1D0) &
                .OR. startMO) .AND. &
                (y(i1) .LT. sto_threshold)) then
              if (nChangeSpe(i1) .GT. 0) then
                if ((i - idxChangeSpe(nChangeSpe(i1), i1)) .LT. 5) then
                  cycle
                end if
