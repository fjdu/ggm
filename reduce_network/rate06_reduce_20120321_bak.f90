...
!     do i=1, nSpecies
!       write (*,*) nameSpecies(i), n_cons_reacs(i), n_prod_reacs(i)
!       do j=1, n_cons_reacs(i)
!         i1 = cons_reacs(j, i)
!         write (*,*) "   ", i1, strReactants(1:2,i1), " -> ", &
!           strProducts(1:4,i1)
!       end do
...
!         '(5X, 7(A12), ES8.2E2, F9.2, F9.1, 2F6.0, I3, X, A1, X, A2)', &
          IOSTAT=ios) &
      ...
      do i=1, nSpecies
        if (flag_imp_species(i)) then
          str_format = '(A12, 2ES16.6E3, 3(4X, F8.2), " *")'
        else
          str_format = '(A12, 2ES16.6E3, 3(4X, F8.2))'
        end if
        ...
        write (UNIT=fU, FMT=str_format, &
          ...
          (yHistory(i,nHistoryLen)-y_final_cmp(i)) / &
            (y_final_cmp(i) + 1D-100), &
